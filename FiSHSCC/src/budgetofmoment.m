...
%     SCC = F{11};
    shearmodulusfrominput=mu_opt;
    straindropfrominput=straindrop_opt;
end
%%% CASE 2
if (ShearModulusFromInputFile==1) & (StrainDropFromInputFile~=1)
    inputdata=imfile;
...
%     data=[(1:nfault)',F{3:10}];
%     SCC = F{11};
    shearmodulusfrominput=(ShearModulusFromInputFile)*10^10;
    straindropfrominput=straindrop_opt;
end
% %%% CASE 3
if (ShearModulusFromInputFile~=1) & (StrainDropFromInputFile==1)
...
%     SCC = F{11};
    shearmodulusfrominput=mu_opt;
    straindropfrominput=(StrainDropFromInputFile)*10^-5;
end
% 
% %%% CASE 4
if (ShearModulusFromInputFile==1) & (StrainDropFromInputFile==1)
    inputdata=imfile;
%     fid = fopen(inputdata);
%     head1=fscanf(fid, '%s %s %s %s %s %s %s %s %s %s %s %s %s\n',12);
%     F=textscan(fid,'%s %s %f %f %f %f %f %f %f %f %f %f %f');
%     fault_name=F{1};
%     faultname=char(fault_name);
%     nfault=size(faultname(:,:),1);
%     ScaleRel=F{2};
...
ScR = [DATA.(fault_name{i}).ScR];
Length = [DATA.(fault_name{i}).Length];
Dip = [DATA.(fault_name{i}).Dip];
Seismogenic_thickness = [DATA.(fault_name{i}).Seismogenic_Thickness];
Slipmin = [DATA.(fault_name{i}).SRmin];
Slipmax = [DATA.(fault_name{i}).SRmax];
mag = [DATA.(fault_name{i}).Mobs];
sdmag = [DATA.(fault_name{i}).sdMobs];
Last_eq_time = [DATA.(fault_name{i}).Last_eq_time];
SCC = [DATA.(fault_name{i}).SCC];
ShearModulusFromInputFile = [DATA.(fault_name{i}).ShearModulus];
StrainDropFromInputFile = [DATA.(fault_name{i}).StrainDrop];
yfc = [DATA.(fault_name{i}).year_for_calculations];

%%%%%%%%%%%%%%%%%%% Year for calculations

% if isempty(year_for_calculations)
%     warning('Year: You are using current year for computing elapsed time')
% %     year_for_calculations=clock;
% %     yfc=year_for_calculations(1);
% else
...
Telap=yfc-Last_eq_time;
mu=shearmodulusfrominput;
straindrop=straindropfrominput;
...
%%%% MAIN BLOCK 1  calculate Mmax with a standard deviation    %%%%
    ...
    dM1=[dMMO; ar_coeff(3); dMRLD; dMRA];
    dM1=round(dM1*100)/100; %round to second decimal
    %M_dM_Lengths_forLOGoutput(i,:)=[Length/1000,LAR/1000, M',dM1'];

    M(isnan(M))=[]; % if a magnitude is equal to NaN then it is not used
    dM1(isnan(dM1))=[]; % if a magnitude is equal to NaN then it is not used
    Mmean=mean(M);
    M1=mag-Mmean;
    ...
    if mag<Mmean
        ...
        elseif abs(mag-Mmean)>.5
            sdmag=mean(dM1)+.2*abs(sdmag-mean(dM1));
        end
    elseif abs(mag-Mmean)>1
        disp('Warning:Please consider revising the geometery parameters.')
    end
    ...
    faultname = cell2mat(fault_name(i));
    ...
    conflated = conflate_pdfs2(x_range_of_mag, pdf_magnitudes);
    [Mmax,sigma_Mmax] = normfit(x_range_of_mag,[],[],summed_pdf_magnitudes);
    pdf_test = normpdf(x_range_of_mag, Mmax, sigma_Mmax);
    % figure
    % plot(x_range_of_mag, pdf_magnitudes)
    % hold on
    % plot(x_range_of_mag, conflated, '--bla')
    % hold on
    % plot(x_range_of_mag, pdf_test, '--c')
    % % legend('MMO', 'MAR', 'MRLD','Observed M','conflated', 'Fitted Gaussian')
    ...
    if LAR<Length
        plot(x_range_of_mag,pdf_magnitudes(count_pdf,:),'LineWidth',1.0,'LineStyle','*','Color','y')
        count_pdf=count_pdf+1;
    end
    ...
    plot(x_range_of_mag, conflated, ...
        'LineWidth',2.0,'LineStyle','-.','Color','g')

    stem(Mmean,max(summed_pdf_magnitudes),'y','LineWidth',1.5)

    [aa, bb]=max(conflated);
    Mmax1=x_range_of_mag(bb);
    stem(Mmax1,max(conflated),'r','LineWidth',1.5)
    line([(Mmax1-sigma_Mmax) (Mmax1+sigma_Mmax)], [max(conflated) max(conflated)],...
    ...
    % hold on
    % plot(x_range_of_mag, pdf_test, '--c')
    legend('MMO', 'MAR', 'MRLD','MObs','SumD','Mmax','','Conflated', 'Mmean Empricals', 'Mmax-Conf')
...
figname=strcat('./output_files/', outputname,'_MB_',fault_name{i});
...
fprintf(fidout,'%s\n',fault_name{i});
    ...
    % fprintf('%1s',blanks(1));
    % fprintf('%s\n',fault_name);
    MB_output= sprintf('%d %3.1f %3.1f %d %4.2f %d %0.4e %1s %s', out(i,:),blanks(1), fault_name{i});



DATA.(fault_name{i}).id=i;
DATA.(fault_name{i}).Mmax=output_Mmax_sigmaMmax(i,1);
DATA.(fault_name{i}).sdMmax=output_Mmax_sigmaMmax(i,2);
DATA.(fault_name{i}).Tmean=Tmean;
DATA.(fault_name{i}).CV=alfa;
DATA.(fault_name{i}).Telap=Telap;
DATA.(fault_name{i}).MomentRate=MomentRate(i,:);
...
valuesCell = struct2cell(DATA);
valuesCell = struct2cell(DATA.Paganica);
valuesCell = valuesCell(2:end, :);

% Convert the cell array to a matrix
data = (cell2mat(valuesCell))';

% Specify the filename to save the structure
filename = 'DATA_mb.mat';
...
%%% create a LogFileMB.txt for keep trace of your calculations
LogFileMB(inputdata,outputfile,data,ScR,faultname,shearmodulusfrominput,straindropfrominput,M_dM_Lengths_forLOGoutput,out,n_sigma_trunc_magnitudes,yfc)
fclose('all');
