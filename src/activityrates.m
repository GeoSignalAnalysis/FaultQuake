...
nfault=numel(fieldNames); id=[]; mag = []; sdmag = []; 
Tmean=[];alpha_val=[];Telapsed=[]; Morate_input=[];
ProbabilityOfOccurrence=[]; Max_Telap=[]; fault_name = {};

idgr=[]; mt=[];b=[];

% Initialize cell array and vectors    id Mchar sdMchar Tmean alfa Telap Mo_rate(Nmyr-1) name

for i = 1:numel(fieldNames)
    fieldName = fieldNames{i};  subStruct = DATA.(fieldName);   
    id=[id;subStruct.id]; mag=[mag;subStruct.Mmax]; sdmag=[sdmag;subStruct.sdMmax];
    Morate_input=[Morate_input;subStruct.MomentRate];  Tmean=[Tmean;subStruct.Tmean];
    Telapsed=[Telapsed;subStruct.Telap]; alpha_val=[alpha_val;subStruct.CV];
    idgr=[idgr;subStruct.id]; mt=[mt;subStruct.Mmin];b=[b;subStruct.b_value];

    fault_name{i,1} = fieldNames{i};
end
...
fault_name=F{9}; faultname=char(fault_name); nfault=size(faultname(:,:),1);
data=[F{1},F{2},F{3},F{4},F{5},F{6},F{7},F{8}]; id=data(:,1); mag=data(:,2);sdmag=data(:,3);
Tmean=data(:,4);alpha_val=data(:,5);Telapsed=data(:,6); Morate_input=data(:,7);
ProbabilityOfOccurrence=data(:,8); Max_Telap=max(Telapsed); %useful for figures;
% fclose(fid);

% fieldNames = fieldnames(DATA); nfault=numel(fieldNames); id=[]; mag = []; sdmag = []; 
% Tmean=[];alpha_val=[];Telapsed=[]; Morate_input=[]; ProbabilityOfOccurrence=[]; 
% Max_Telap=[]; fault_name = {};idgr=[]; mt=[];b=[];
% 
% % Initialize cell array and vectors    id Mchar sdMchar Tmean alfa Telap Mo_rate(Nmyr-1) name
% 
% for i = 1:numel(fieldNames)
...
elseif Fault_behaviour==8
    [outputname]=TruncatedGR(c,d,outputname,fault_name,mag,mt,Morate,id,nfault,bin,b);
...
filename = 'DATA_ar.mat';

% Save the structure to a MAT file
save(filename, 'DATA');

LogFileAR(inputdata,outputname,data,faultname(i),Fault_behaviour,MFD,window, binstep, grinputoptions,b)
